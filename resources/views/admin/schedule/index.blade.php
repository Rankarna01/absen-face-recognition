...
<!-- GANTI KE scripts-head AGAR TER-LOAD DI LAYOUT ADMIN -->
@push('scripts-head')
<script src='https://cdn.jsdelivr.net/npm/fullcalendar@6.1.11/index.global.min.js'></script>
<style>
    /* Styling agar kalender sesuai tema Family Market */
    ...
    .fc-event { cursor: pointer; padding: 4px; border-radius: 6px; font-size: 0.75rem; border: none; color: #fff !important; font-weight: 500; }
...
<div x-data="scheduleManager()" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-3">
        <div class="bg-base p-6 rounded-2xl border border-gray-100 shadow-sm">
            <!-- x-ignore sangat penting agar Alpine tidak mengganggu FullCalendar -->
            <div id='calendar' x-ignore class="min-h-[600px] w-full"></div>
            <p class="text-xs text-gray-400 mt-4"><i class="fa-solid fa-circle-info mr-1"></i> Klik pada kotak jadwal di dalam kalender untuk melihat detail atau menghapusnya.</p>
        </div>
    </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('scheduleManager', () => ({
            ...
            assignForm: { user_id: '', shift_id: '', tanggal_mulai: '', tanggal_selesai: '' },
            shiftForm: { nama_shift: '', jam_masuk: '', jam_pulang: '', warna: '#F5A623' },
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            locale: 'id', // Format Bahasa Indonesia
            height: 'auto',
            headerToolbar: {
                ...
                Swal.fire({
                    ...
                    html: `
                        <div class="text-left mt-4 mb-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <p class="mb-2 text-sm"><span class="text-gray-500 w-20 inline-block">Pegawai</span> <strong class="text-secondary">${nama}</strong></p>
                            <p class="mb-2 text-sm"><span class="text-gray-500 w-20 inline-block">Shift</span> <strong class="text-secondary">${shift}</strong></p>
                            <p class="mb-2 text-sm"><span class="text-gray-500 w-20 inline-block">Jam</span> <strong class="text-secondary">${jam}</strong></p>
                            <p class="mb-2 text-sm"><span class="text-gray-500 w-20 inline-block">Tanggal</span> <strong class="text-secondary">${tanggal}</strong></p>
                        </div>
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: (res) => {
                                Toast.fire({ icon: 'success', title: res.message });
                                info.event.remove(); // Hapus langsung dari kalender tanpa reload!
                            }
                        });
                    }
                });
